...
public class MemberDAO {
	 ...
	 * @return
	 */
	public boolean insertReservation(MemberBean bean) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		boolean flag = false;
		...
		try {
			con = pool.getConnection();
			sql = "INSERT INTO nomadMember( id, coffeeClassSubject, coffeeClassTime, coffeeClassTotalPeople ) VALUES( ?, ?, ?, ? )";
			pstmt = con.prepareStatement(sql);

			pstmt.setString(1, bean.getId());
			pstmt.setString(2, bean.getCoffeeClassSubject());
			pstmt.setString(3, bean.getCoffeeClassTime());
			pstmt.setString(4, bean.getCoffeeClassTotalPeople());
